...
EDITING RULES - Clean the content by:
...
CRITICAL FORMATTING PRESERVATION - You MUST:
- PRESERVE EXACTLY every ## heading, ### subheading, #### etc.
- PRESERVE EXACTLY every **bold text**, *italic text*, `code text`
- PRESERVE EXACTLY every bullet point (-, *, +) and numbering (1., 2., etc.)
- PRESERVE EXACTLY every table structure (| column | column |)
- PRESERVE EXACTLY every code block with ```
- PRESERVE EXACTLY every line break, spacing, and indentation
- PRESERVE EXACTLY every link format [text](url)
- PRESERVE EXACTLY every > blockquote
- DO NOT change ANY formatting characters: #, *, _, `, |, >, etc.
- DO NOT add or remove any markdown syntax
- ONLY change the actual text content, never the formatting

EXAMPLE:
If original has: "## **Comprehensive** Model Overview"
Return: "## **Model** Overview" (removed "Comprehensive", kept ## and **)

Original document:
{content}

Return the cleaned markdown with IDENTICAL formatting:"""
...
- KEEP valid JSON syntax with proper quotes, brackets, commas
...
PRESERVE the original formatting exactly. DO NOT change any formatting characters.
...
Return cleaned content with identical formatting:"""
    ...
    def clean_file_content(self, file_path: Path, content: str) -> str:
        """Clean content using LLM with format preservation and validation."""
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    # Validate formatting is preserved for markdown files
                    if not self.validate_formatting_preserved(content, cleaned, file_path):
                        logger.warning(f"Formatting not preserved for {file_path.name}, attempt {attempt + 1}")
                        if attempt < 2:
                            # Try again with even more explicit prompt
                            prompt = self.get_ultra_strict_prompt(file_path, content)
                            time.sleep(1)
                            continue
                        else:
                            logger.error(f"Could not preserve formatting for {file_path.name}, returning original")
                            return content
    ...
    def validate_formatting_preserved(self, original: str, cleaned: str, file_path: Path) -> bool:
        """Validate that essential markdown formatting is preserved."""
        if file_path.suffix.lower() != '.md':
            return True  # Only validate markdown files
        
        # Count key formatting elements
        original_headers = original.count('#')
        cleaned_headers = cleaned.count('#')
        
        original_bold = original.count('**')
        cleaned_bold = cleaned.count('**')
        
        original_italic = original.count('*') - original_bold * 2  # Subtract bold asterisks
        cleaned_italic = cleaned.count('*') - cleaned_bold * 2
        
        original_code = original.count('`')
        cleaned_code = cleaned.count('`')
        
        original_bullets = original.count('\n- ') + original.count('\n* ')
        cleaned_bullets = cleaned.count('\n- ') + cleaned.count('\n* ')
        
        # Check if major formatting elements are preserved
        formatting_preserved = (
            abs(original_headers - cleaned_headers) <= 1 and  # Allow small header differences
            abs(original_bold - cleaned_bold) <= 2 and        # Allow small bold differences  
            abs(original_code - cleaned_code) <= 2 and        # Allow small code differences
            abs(original_bullets - cleaned_bullets) <= 1      # Allow small bullet differences
        )
        
        if not formatting_preserved:
            logger.warning(f"Formatting validation failed for {file_path.name}")
            logger.warning(f"Headers: {original_headers} -> {cleaned_headers}")
            logger.warning(f"Bold: {original_bold} -> {cleaned_bold}")
            logger.warning(f"Code: {original_code} -> {cleaned_code}")
            logger.warning(f"Bullets: {original_bullets} -> {cleaned_bullets}")
    ...
    def remove_unwanted_formatting(self, content: str, file_path: Path) -> str:
        ...
        if file_path.suffix.lower() == '.md':
            ...
            # Remove any other common code block markers
        ...
        # Additional check: if the content starts with something like "Here's the cleaned..." remove it
        lines = content.split('\n')
        if lines and ('here' in lines[0].lower() or 'cleaned' in lines[0].lower()) and len(lines) > 1:
            # Skip the first line if it looks like an LLM introduction
    ...
    def get_ultra_strict_prompt(self, file_path: Path, content: str) -> str:
        """Ultra-strict prompt for when normal formatting preservation fails."""
        if file_path.suffix.lower() == '.md':
            return f"""
CRITICAL: You are ONLY allowed to change word content, NEVER formatting.

FORBIDDEN CHANGES:
- DO NOT remove any # ## ### headers
- DO NOT remove any **bold** or *italic* text
- DO NOT remove any `code` text  
- DO NOT remove any bullet points (-, *, +)
- DO NOT remove any numbers (1., 2., 3.)
- DO NOT remove any tables | column |
- DO NOT remove any links [text](url)
- DO NOT change spacing or line breaks

ALLOWED CHANGES:
- Remove adjectives like "comprehensive", "robust", "innovative"
- Remove filler phrases like "it should be noted"
- Replace "training" with "RAG/API" concepts
- Make statements more direct

EXAMPLE - ONLY change words, keep ALL formatting:
Original: "## **Comprehensive** Model Training Overview"
Correct: "## **Model** RAG Overview"
Wrong: "Model RAG Overview" (removed ## and **)

Original content (preserve ALL formatting exactly):
{content}

Return with IDENTICAL formatting:"""
        else:
            return self.get_cleaning_prompt(file_path, content)
    
    def clean_target_files(self) -> Dict[str, int]:
        """Clean only .json and .md files, ignore all others."""
        stats = {'cleaned': 0, 'skipped': 0, 'errors': 0, 'empty_retries': 0, 'format_fallbacks': 0}
        ...
        for source_file in target_files:
            try:
                ...
                try:
                    ...
                    # Check if we fell back to original due to formatting issues
    ...
    def run(self) -> Dict[str, int]:
        ...
        logger.info(f"Format fallbacks: {stats['format_fallbacks']}")
...
if __name__ == "__main__":
    ...
    print(f"🛡️  Format fallbacks: {stats['format_fallbacks']} files")
    print(f"❌ Errors: {stats['errors']} files")
