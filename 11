...
"""
    ...
    def remove_markdown_wrapper(self, content: str, file_path: Path) -> str:
        """Remove markdown code block wrappers from LLM output while preserving original markdown formatting."""
        content = content.strip()
        
        # Only remove if the ENTIRE content is wrapped in a single code block
        if content.startswith('```') and content.endswith('```'):
            lines = content.split('\n')
            
            # Check if it's a complete wrapper (starts with ``` and ends with ```)
            if len(lines) >= 3 and lines[0].startswith('```') and lines[-1].strip() == '```':
                # Remove opening wrapper line (```markdown, ```json, ```md, etc.)
                lines = lines[1:]
                
                # Remove closing wrapper line
